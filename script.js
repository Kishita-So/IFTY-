      ...
      ${folder.collapsed ? '' : `
        ...
        ">
          ...
          <input
            ...
            onkeydown="if(event.key==='Enter'){ addWordToFolder('${folder.id}'); }"
            style="
              flex: 1;
              padding: 8px;
              border: 1px solid #cbd5e1;
...
function applyWordData(wordObj, data) {
  ...
  if (data.pronunciation) {
    wordObj.pronunciation = data.pronunciation;
  }

  if (data.partOfSpeech) {
    wordObj.partOfSpeech = data.partOfSpeech;
  }

  if (data.transitivity) {
    wordObj.transitivity = data.transitivity;
  }

  if (data.countability) {
    wordObj.countability = data.countability;
  }

  if (data.details) {
    wordObj.details = data.details;
  }

  if (Array.isArray(data.derivatives)) {
    wordObj.derivatives = data.derivatives;
  }

  if (data.forms && typeof data.forms === 'object') {
    wordObj.forms = data.forms;
  }
}
...
window.openEditWordModal = function(folderId, wordIndex) {
  ...
  const meaningsStr = Array.isArray(w.meanings)
    ? w.meanings.join('\n')
    : (w.meanings || '');

  const examplesStr = w.examples
    ? w.examples
        .map(ex => `${ex.en || ''} | ${ex.ja || ''}`)
        .join('\n')
    : '';

  const derivativesStr = Array.isArray(w.derivatives)
    ? w.derivatives.join('\n')
    : (w.derivatives || '');
...
window.saveEditedWord = function(folderId, wordIndex) {
  ...
  const wordVal =
    document.getElementById("editWordText").value.trim();

  const pronunciationVal =
    document.getElementById("editPronunciationText").value.trim();

  const partOfSpeechVal =
    document.getElementById("editPartOfSpeechText").value.trim();

  const usageVal =
    document.getElementById("editUsageText").value.trim();

  const meaningsVal =
    document.getElementById("editMeaningsText")
      .value
      .split('\n')
      .map(s => s.trim())
      .filter(Boolean);
  ...
  const examplesRaw =
    document.getElementById("editExamplesText")
      .value
      .split('\n')
      .map(s => s.trim())
      .filter(Boolean);
  ...
  const derivativesVal =
    document.getElementById("editDerivativesText")
      .value
      .split('\n')
      .map(s => s.trim())
      .filter(Boolean);
  ...
  const detailsVal =
    document.getElementById("editDetailsText")
      .value
      .trim();
...
window.switchToChatView = function() {
  ...
  const vocabPage =
    document.getElementById("vocabPage");

  const aiChatPage =
    document.getElementById("aiChatPage");
...
window.createNewChatSession = function() {
  ...
  const newSession = {
    ...
    messages: [
      {
        role: 'assistant',
        text: 'こんにちは！ALLIAアシスタントです。何でも聞いてください！'
      }
    ]
  };
